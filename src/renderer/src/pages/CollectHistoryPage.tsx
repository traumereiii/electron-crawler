...
import TaskTab from '@renderer/components/history/task-tab/TaskTab'
import ParsingTab from '@renderer/components/history/parsing-tab/ParsingTab'
import { useHistoryTab } from '@renderer/store/history/history-tab'
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 mt-1">세션별, URL별, 파싱 결과 및 에러 현황</p>
        </div>
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as HistoryTab)} className="w-full">
        ...
        {/* Parsing Tab */}
        <TabsContent value="parsing" className="mt-5 space-y-5">
          <ParsingTab></ParsingTab>
        </TabsContent>

        {/* Errors Tab */}
        <TabsContent value="errors" className="mt-5 space-y-5">
          {/* Summary Cards */}
              ...
              <div className="space-y-3">
                {/*collectTasks
                  ...
                  .map((task) => (
                    <div
                      key={task.id}
                      className="p-4 bg-red-50 rounded-xl border border-red-100 hover:bg-red-100 transition-colors cursor-pointer"
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <Badge className="bg-red-200 text-red-900 hover:bg-red-200 border-0">
                              {task.errorType}
                            </Badge>
                          ...
                          <p className="text-slate-900 text-sm mb-1">{task.url}</p>
                          <p className="text-slate-600 text-xs">{task.startedAt}</p>
                        </div>
